...
class CurrentModelInfo(Widget):
  def __init__(self):
    ...
    default_text = f"{DEFAULT_MODEL} {tr('(Default)')}".lower()
...
class ModelsLayoutMici(NavScroller):
  ...
  def _show_folders(self):
    ...
    folder_buttons = []
    default_btn = BigButton(f"{DEFAULT_MODEL} {tr('(Default)')}".lower())
  ...
  def _update_state(self):
    ...
    model_text = manager.activeBundle.displayName.lower() if manager.activeBundle.index > 0 else f"{DEFAULT_MODEL} {tr('(Default)')}".lower()
