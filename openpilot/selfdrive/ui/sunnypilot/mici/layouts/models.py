...
class CurrentModelInfo(Widget):
  def __init__(self):
    ...
    default_text = f"{get_default_model()} {tr('(Default)')}".lower()
...
class ModelsLayoutMici(NavScroller):
  ...
  def _show_folders(self):
    ...
    folder_buttons = []
    default_btn = BigButton(f"{get_default_model()} {tr('(Default)')}".lower())
  ...
  def _update_state(self):
    ...
    model_text = ((ui_state.active_bundle or {}).get("displayName") or f"{get_default_model()} {tr('(Default)')}").lower()
